...
from models.DACE import DACE
try:
    from models.DyGKT_KC import DyGKT_KC
except ImportError:
    DyGKT_KC = None
from models.IKDG_KT import IKDG_KT
...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        if args.model_name == 'DKT':
        ...
        elif args.model_name == 'DACE':
            dynamic_backbone = DACE(node_raw_features=node_raw_features,
                                    edge_raw_features=edge_raw_features,
        ...
        elif args.model_name == 'DyGKT_KC':
            if DyGKT_KC is None:
                raise ImportError('DyGKT_KC is unavailable because models/DyGKT_KC.py is not present.')
            # 尝试从mapping_info.pkl中读取节点类型信息
            num_students, num_questions, num_kcs = None, None, None
            try:
                import pickle
                import os
                mapping_info_path = f'./processed_data/{args.dataset_name}/mapping_info.pkl'
                if os.path.exists(mapping_info_path):
                    with open(mapping_info_path, 'rb') as f:
                        mapping_info = pickle.load(f)
                        if 'student_to_node_id' in mapping_info:
                            num_students = len(mapping_info['student_to_node_id'])
                        if 'question_to_node_id' in mapping_info:
                            num_questions = len(mapping_info['question_to_node_id'])
                        if 'kc_to_node_id' in mapping_info:
                            num_kcs = len(mapping_info['kc_to_node_id'])
                        logger.info(f'Loaded node type info: num_students={num_students}, num_questions={num_questions}, num_kcs={num_kcs}')
            except Exception as e:
                logger.warning(f'Failed to load mapping_info.pkl: {e}, using default values')
            
            dynamic_backbone = DyGKT_KC(node_raw_features=node_raw_features,
                                        edge_raw_features=edge_raw_features,
                                        dropout=args.dropout, 
                                        num_neighbors=args.num_neighbors,
                                        device=args.device,
            ...
            dynamic_backbone = IKDG_KT(node_raw_features=node_raw_features,
                                        edge_raw_features=edge_raw_features,
                                        dropout=args.dropout, 
                                        num_neighbors=args.num_neighbors,
                                        device=args.device,
                                        ...
                                        use_time_decay_in_attention=args.use_time_decay_in_attention,
        ...
        if args.model_name in ['DyGKT', 'DyGKT_KC', 'IKDG-KT', 'DACE']:
            if args.dataset_name == 'dbe_kt22':
                link_predictor = MergeLayer(input_dim1=64, input_dim2=64, hidden_dim=64, output_dim=1)
            else:
                link_predictor = MergeLayer(input_dim1=64, input_dim2=64, hidden_dim=64, output_dim=1)
        else:
            link_predictor = MergeLayer(node_raw_features.shape[1], input_dim2=node_raw_features.shape[1], hidden_dim=node_raw_features.shape[1], output_dim=1)
        ...
        for epoch in range(args.num_epochs):
            if args.test and epoch > 0:
                ...
                sys.exit()
            model.train()
            if args.model_name in ['DyGKT','DyGKT_KC','IKDG-KT','DACE','QIKT','IEKT','IPKT','DIMKT', 'TGAT', 'TGN', 'DyGFormer','DKT','AKT','CTNCM','simpleKT']:
                # training, only use training graph
                model[0].set_neighbor_sampler(train_neighbor_sampler)
            if args.model_name == 'IKDG-KT':
                # Initialize the concept-history manager once per training run.
                # 只在第一次创建，历史在整个训练过程中累积
                if not hasattr(model[0], 'kc_history_manager') or model[0].kc_history_manager is None:
                    kc_history_manager = KCHistoryManager(device=args.device)
                    model[0].set_kc_history_manager(kc_history_manager)
                    logger.info('KC history manager initialized for IKDG-KT')
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.model_name == 'IKDG-KT':
                    # IKDG-KT predicts only student-question response edges.
                    # 1. 前向传播（包含结构更新）
                    src_emb, dst_emb, kc_emb, struct_loss = model[0].forward_with_kc(
                    ...
                    # 2. 识别 S-Q 边（需要预测答题结果的边）
                    is_sq = model[0].is_student_question_edge(batch_src_node_ids, batch_dst_node_ids)
                    
                    # 3. 计算预测损失（仅对 S-Q 边）
                    if is_sq.any():
                        # 筛选 S-Q 边的嵌入和标签
                        src_sq = src_emb[is_sq]
                        dst_sq = dst_emb[is_sq]
                        kc_sq = kc_emb[is_sq] if kc_emb is not None else None
                        
                        # 预测
                        predicts = model[0].gated_predictor(src_sq, dst_sq, kc_sq).squeeze(dim=-1).sigmoid()
                        
                        # Fix: batch_edge_labels is numpy, is_sq is cuda tensor. 
                        # Convert labels to tensor first, then index with mask
                        all_labels_tensor = torch.tensor(batch_edge_labels, dtype=torch.float32, device=args.device)
                        labels_sq = all_labels_tensor[is_sq]
                        
                        # BCE Loss
                        bce_loss = loss_func(predicts, labels_sq)
                        
                        # 记录预测结果（仅记录有效预测）
                        train_predicts.append(predicts)
                        train_labels.append(labels_sq)
                    else:
                        bce_loss = torch.tensor(0.0, device=args.device)
                        # 如果没有 S-Q 边，不记录 predicts
                    
                    # 4. 总损失
                    loss = bce_loss + model[0].lambda_struct * struct_loss
                    ...
                    train_idx_data_loader_tqdm.set_description(f'Epoch: {epoch + 1}, train for the {batch_idx + 1}-th batch, train loss: {loss.item():.4f}, struct: {struct_loss.item():.4f}')
                    continue

                if args.model_name in ['DyGKT','DyGKT_KC','DACE','DKT','AKT','CTNCM','simpleKT']:
