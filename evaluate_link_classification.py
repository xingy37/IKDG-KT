...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        if args.model_name == 'DKT':
            ...
            try:
                ...
                if os.path.exists(mapping_info_path):
                    ...
                    logger.info(
                        'Loaded node type info: num_students=%s, num_questions=%s, num_kcs=%s',
                        num_students,
                        num_questions,
                        num_kcs,
                    )
            except Exception as error:
                logger.warning('Failed to load mapping_info.pkl: %s; using default values', error)

            dynamic_backbone = IKDG_KT(
        ...
        if args.model_name in ['DyGKT', 'DyGKT_KC', 'IKDG-KT'] and args.dataset_name == 'dbe_kt22':
            link_predictor = MergeLayer(input_dim1=64, input_dim2=64, hidden_dim=64, output_dim=1)
        elif args.model_name in ['DyGKT', 'DyGKT_KC', 'IKDG-KT']:
            link_predictor = MergeLayer(input_dim1=64, input_dim2=64, hidden_dim=64, output_dim=1)
        else:
